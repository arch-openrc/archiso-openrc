...
iso_name=archlinux-openrc-grub
iso_label="ARCH_$(date +%Y%m)"
iso_version=$(date +%Y.%m.%d)
install_dir=arch-openrc
work_dir=work
out_dir=out
gpg_key=""
...
make_efi() {
    ...
    cp ${script_path}/efiboot/x86_64-loader.efi ${work_dir}/iso/EFI/boot/loader.efi
    cp ${script_path}/efiboot/i386-loader.efi ${work_dir}/iso/EFI/boot/bootia32.efi
	sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
		${script_path}/efiboot/grub.cfg > ${work_dir}/iso/EFI/boot/grub.cfg

	ln -s boot ${work_dir}/iso/EFI/Boot
	ln -s boot ${work_dir}/iso/EFI/BOOT
...
make_efiboot() {
    ...
    cp ${script_path}/efiboot/x86_64-loader.efi ${work_dir}/efiboot/EFI/boot/loader.efi
    cp ${script_path}/efiboot/i386-loader.efi ${work_dir}/efiboot/EFI/boot/bootia32.efi
	sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
		${script_path}/efiboot/grub.cfg > ${work_dir}/iso/EFI/boot/grub.cfg
...
# Create grub standalone efi files
make_grubefi() {
	#echo 'configfile ${cmdpath}/grub.cfg' > /tmp/grub.cfg
	sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
		${script_path}/efiboot/grub.cfg > /tmp/grub.cfg

	grub-mkstandalone -d /usr/lib/grub/${arch}-efi/ -O ${arch}-efi --modules="part_gpt part_msdos" --fonts="unicode" --locales="en@quot" --themes="" -o "${script_path}/efiboot/${arch}-loader.efi"  "boot/grub/grub.cfg=/tmp/grub.cfg" -v
}

# Build OpenRC
make_openrc() {
	setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.openrc)" install
}
